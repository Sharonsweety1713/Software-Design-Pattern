  ...
  const handleSubmit = async (e) => {
    ...
    if (validateStep()) {
      ...
      try {
        ...
        setTimeout(() => {
          alert("Payment Successful,Your Policy Will be added soon");
          setFormData({
            PlanName: '',
            Period: '',
            Amount: '',
      ...
      <Card className="p-8 rounded-lg shadow-lg w-full max-w-4xl bg-card text-card-foreground border border-black dark:border-white">
        ...
        <CardFooter>
          <div className="flex justify-between">
            {step > 1 && (
              <button type="button" onClick={prevStep} className="px-4 py-2 bg-gray-500 w-64   text-white rounded">
                Previous
              </button>
            )}
            {step < 5 ? (
              <div className="ml-72">
              <button type="button" onClick={nextStep} className=" w-64 px-4 py-2 bg-customGreen  text-white rounded">
                Next
              </button>
              </div>
            ) : (
              <div className="ml-72">
              <button type="submit" onClick={handleSubmit} className="px-4 w-64 py-2 bg-customGreen text-white rounded">
                Submit
              </button>
              </div>
            )}
